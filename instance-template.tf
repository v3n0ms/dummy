...
sudo docker run -p 8080:8080 -d -e DB_USER=${local.envs["USER"]} -e DB_NAME=${local.envs["NAME"]} -e DB_PASSWORD=${local.envs["PASSWORD"]} -e DB_HOST=${local.envs["HOST"]} asia.gcr.io/dummy-project-365407/pos
EOF
