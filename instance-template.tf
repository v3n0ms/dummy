...
sudo docker run -p 8080:8080 -d -e DB_USER=${local.envs["USER"]} -e DB_NAME=${local.envs["NAME"]} -e DB_PASSWORD=${local.envs["PASSWORD"]} -e DB_HOST=${google_sql_database_instance.private_network.id} asia.gcr.io/dummy-project-365407/pos
EOF
